...
class Evaluator:
    ...
    def _get_data_from_json(self):
        ...
        for order in data:
            for sentence_index in range(len(order["order"])):
                if order["order"][sentence_index][0]:
                    order["order"][sentence_index][0] = "[USER]"
                else:
                    order["order"][sentence_index][0] = "[ADVISOR]"
    ...
    def write_log(self, eval_type, TP, FP, FN, TP_W, FP_W, FN_W):
        ...
        if self.config["SLIDING_WIN_SIZE"]==0:
            ...
            jfr = open("log/{}_birth.json".format(model_path), "r")
            ...
            jfr.close()
        with open("log/{}_birth.json".format(model_path), "w") as fout:
    ...
    def write_badcase(self):
        ...
        with open("badcases_birth/{}_Win{}_{}.txt".format(model_path, win_size, input_type),  "w") as bf:
...
def main():
    ...
    with open("config_birth.yaml","r") as stream:
